...
subroutine map2alm_pol_d(nsmax, nlmax, nmmax, map_TQU, alm_TGC, zbounds, w8ring_TQU)
...
subroutine map2alm_pol_pre1_d(nsmax, nlmax, nmmax, map_TQU, alm_TGC, zbounds, w8ring_TQU, plm)
    !=======================================================================
    !     computes the a(l,m) from a polarized map for the HEALPIX pixelisation
...
!$OMP      keep_north, keep_south, map_TQU) &
!$OMP  private(ithl, nphl, istart_north, istart_south, ith, ring, i, ii, status)
       if (do_openmp()) then
          allocate(ring(0:nphmx-1),stat = status)
          call assert_alloc(status,code,'ring')
       endif
...
!$OMP parallel default(none) &
!$OMP shared(nlmax, nmmax, lchk, uchk, omega_pix, n_lm, plm, ith, cth, sth, keep_it, normal_l, alm_TGC, phas_ns, one_on_s2, c_on_s2) &
!$OMP private(dalm, lam_fact, phas_sd, phas_sdx, phm, php, status, &
!$OMP   m, fm2, normal_m, ithl, l_min, par_lm, i_mm, lam_lm, lam_lm1m, lambda_w, lambda_x, &
!$OMP   cth_ring, fm_on_s2, two_on_s2, two_cth_ring, a_w, a_x, b_w, l, fl, flm1, i)
          ...
          do ithl = 0, uchk - lchk
             ...
             if (keep_it(ithl) .and. nlmax >= l_min) then ! avoid un-necessary calculations (EH, 09-2001)
                ...
                if (m >= l_min) then
                   ...
                   dalm(0:1, m) = dalm(0:1, m) + lam_lm * phas_sd(par_lm:par_lm+1)
                   dalm(2:5, m) = dalm(2:5, m)  &
                        &       - lambda_w * phas_sd(par_lm+2:par_lm+5) + lambda_x * phas_sdx(par_lm:par_lm+3)
                endif
                ...
                do l = m+1, nlmax
                   ...
                   if (l >= l_min) then
                      ...
                      dalm(0:1, l) = dalm(0:1, l) + lam_lm * phas_sd(par_lm:par_lm+1)
                      dalm(2:5, l) = dalm(2:5, l) - lambda_w * phas_sd(par_lm+2:par_lm+5) &
                           &       + lambda_x * phas_sdx(par_lm:par_lm+3)
                   endif
...
!$OMP end parallel
  ...
  subroutine map2alm_pol_pre2_d(nsmax, nlmax, nmmax, map_TQU,alm_TGC, zbounds, w8ring_TQU, plm)
    !=======================================================================
    !     computes the a(l,m) from a polarized map for the HEALPIX pixelisation
...
!$OMP parallel default(none) &
!$OMP shared(nsmax, nlmax, nmmax, npix, nrings, nphmx, lchk, uchk,nph, startpix, kphi0, w8ring_TQU, phas_ns, keep_north, keep_south, map_TQU) &
!$OMP private(ithl, nphl, istart_north, istart_south, ith, ring, i, ii, status)
       if (do_openmp()) then
          allocate(ring(0:nphmx-1),stat = status)
          call assert_alloc(status,code,'ring')
...
!$OMP parallel default(none) &
!$OMP shared(nlmax, nmmax, lchk, uchk, omega_pix, n_lm, ith, sth, keep_it, alm_TGC, phas_ns, plm) &
!$OMP private(dalm, phas_sd, phas_sdx, plm_sub, phm, php, status, m, ithl, l_min, l_start, l, i_mm, i_up, i)
       if (do_openmp()) then
          allocate(dalm(0:5,0:nlmax), plm_sub(1:3,0:nlmax), stat = status)
          call assert_alloc(status,code,'dalm & plm_sub')
       endif
          ...
          do ithl = 0, uchk - lchk
             ...
             if (keep_it(ithl) .and. nlmax >= l_min) then ! avoid un-necessary calculations (EH, 09-2001)
                ...
                if (m >= l_min) then
                   ...
                   dalm(2:5, m) = dalm(2:5, m) - plm_sub(2,m) * phas_sd(5:8) + plm_sub(3,m) * phas_sdx(3:6)
                endif
                !           ---------- l > m ----------
                l_start = max(m+1, l_min) ! odd values of (l+m)
                if (mod(m+l_start,2) == 0) l_start = l_start+1
                do l = l_start, nlmax, 2
                   ...
                   dalm(2:5,l) = dalm(2:5,l) - plm_sub(2,l) * phas_sd(-1:2) + plm_sub(3,l) * phas_sdx(-3:0)
                enddo ! loop on l
                ...
                do l = l_start, nlmax, 2
                   ...
                   dalm(2:5,l) = dalm(2:5,l) - plm_sub(2,l) * phas_sd(5:8) + plm_sub(3,l) * phas_sdx(3:6)
                enddo ! loop on l
