...
root = '/global/u1/t/toshiyan/trash/tmp/'
...
for i in range(mcnum):
    ...
    try:
        plm = pickle.load(open(root+'phi'+str(i)+'.pkl',"rb"))
    except:
        plm = curvedsky.utils.gauss1alm(lmax,ucl[3,:])
        pickle.dump((plm),open(root+'phi'+str(i)+'.pkl',"wb"),protocol=pickle.HIGHEST_PROTOCOL)


    # lensed CMB alms
    try:
        Trlm, Erlm, Brlm = pickle.load(open(root+'lcmb'+str(i)+'.pkl',"rb"))
    except:
        Talm, Ealm = curvedsky.utils.gauss2alm(lmax,ucl[0,:],ucl[1,:],ucl[2,:])
        beta = curvedsky.delens.phi2grad(nside,lmax,plm)
        Trlm, Erlm, Brlm = curvedsky.delens.remap_tp(nside,lmax,beta,np.array((Talm,Ealm,0*Ealm)))
        pickle.dump((Trlm,Erlm,Brlm),open(root+'lcmb'+str(i)+'.pkl',"wb"),protocol=pickle.HIGHEST_PROTOCOL)
    ...
    # lens norm
    try:
        Ag0, Ag1 = np.loadtxt(root+'al.dat',unpack=True,usecols=(1,2))
    except:
        ...
        np.savetxt(root+'al.dat',np.array((np.linspace(0,lmax,lmax+1),Ag0,Ag1)).T)

    # reconstruction
    try:
        plm0, plm1 = pickle.load(open(root+'qrec'+str(i)+'.pkl',"rb"))
    except:
        ...
        plm0 *= Ag0[:,None] 
        plm1 *= Ag1[:,None] 
        pickle.dump((plm0,plm1),open(root+'qrec'+str(i)+'.pkl',"wb"),protocol=pickle.HIGHEST_PROTOCOL)
...
np.savetxt(root+'resbb.dat',np.concatenate((L[None,:],np.mean(cls,axis=0))).T)
