...
def al(p,f,r):
    ...
    for q in p.qlist:
        ...
        if p.qtype=='rot':
            ...
            if q=='TB': Ag = curvedsky.norm_rot.qtb(p.lmax,p.rlmin,p.rlmax,r.lcl[3,:],r.oc[0,:],r.oc[2,:])
            if q=='EB': Ag = curvedsky.norm_rot.qeb(p.lmax,p.rlmin,p.rlmax,r.lcl[1,:],r.oc[1,:],r.oc[2,:])

        if p.qtype=='tau':
            Ac = np.zeros(p.lmax+1)
            if q=='TT': Ag = curvedsky.norm_tau.qtt(p.lmax,p.rlmin,p.rlmax,r.lcl[0,:],r.oc[0,:])
            if q=='EB': Ag = curvedsky.norm_tau.qeb(p.lmax,p.rlmin,p.rlmax,r.lcl[1,:],r.oc[1,:],r.oc[2,:])
...
def qrec(p,falm,fquad,r,diaginv=True):
    ...
    for i in range(p.snmin,p.snmax):
        ...
        for q in p.qlist:
            ...
            if p.qtype=='tau':
                if q=='TT':  glm = curvedsky.rec_tau.qtt(p.lmax,p.rlmin,p.rlmax,r.lcl[0,:],Talm,Talm,nside=p.nsidet)
                if q=='EB':  glm = curvedsky.rec_tau.qeb(p.lmax,p.rlmin,p.rlmax,r.lcl[1,:],Ealm,Balm,nside=p.nsidet)
                clm = glm*0.
