...
def qttte(est,lmax,rlmin,rlmax,fCTT,fCTE,OCT,OCE,OCTE,lfac=''):
    ...
    """
    return lib_norm.norm_quad.qttte(est,lmax,rlmin,rlmax,fCTT,fCTE,OCT,OCE,OCTE,lfac)
...
def qttee(est,lmax,rlmin,rlmax,fCTT,fCEE,OCT,OCE,OCTE,lfac=''):
    ...
    """
    return lib_norm.norm_quad.qttee(est,lmax,rlmin,rlmax,fCTT,fCEE,OCT,OCE,OCTE,lfac)
...
def qteee(est,lmax,rlmin,rlmax,fCEE,fCTE,OCT,OCE,OCTE,lfac=''):
    ...
    """
    return lib_norm.norm_quad.qteee(est,lmax,rlmin,rlmax,fCEE,fCTE,OCT,OCE,OCTE,lfac)
...
def qtbeb(est,lmax,rlmin,rlmax,fCEE,fCBB,fCTE,OCT,OCE,OCB,OCTE,lfac=''):
    ...
    """
    return lib_norm.norm_quad.qtbeb(est,lmax,rlmin,rlmax,fCEE,fCBB,fCTE,OCT,OCE,OCB,OCTE,lfac)
...
def qeb_iter(lmax,elmax,rlmin,rlmax,dlmin,dlmax,CE,OCE,OCB,Cpp,iter=1,conv=0.00001):
    ...
    """
    return lib_norm.norm_quad.qeb_iter(lmax,elmax,rlmin,rlmax,dlmin,dlmax,CE,OCE,OCB,Cpp,iter,conv)


def qall_iter(lmax,rlmin,rlmax,ucl,lcl,ocl,eb='qe'):
    """   
    # Reconstruction Noise
    # TT, TE, EE, TB, EB, EE+EB, TT+TE+EE+EB
    """
    ...
    Ag[:6,:], Ac[:6,:], nlg, nlc = qall('lens',[True,True,True,False,False,False],lmax,rlmin,rlmax,lcl,ocl)
    Ag[3,:], Ac[3,:] = qtb('lens',lmax,rlmin,rlmax,lcl[3,:],ocl[0,:],ocl[2,:])
    if eb=='qe': #QE reconstruction for EB
        Ag[4,:], Ac[4,:] = qeb('lens',lmax,rlmin,rlmax,lcl[1,:],ocl[1,:],ocl[2,:])
    else: #iterative reconstruction for EB
        Ag[4,:], Ac[4,:] = qeb_iter(lmax,rlmax,rlmin,rlmax,rlmin,rlmax,ucl[1,:],ocl[1,:],ocl[2,:],ucl[3,:],iter=10,conv=1e-3)
